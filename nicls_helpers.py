...
# ML stuff
from classifier_io import (
    ClassifierModel,
)  # custom class for getting classifier weights from json
...
def NestedLOGO(
    ...
    estimator,
    param_grid,
    inner_cv_group="session",
    outer_cv_group="session",
    client=None,
    verbose=True,
):
    ...
    for train_idx, test_idx in outer_cv.split(
        X=data.data, y=data.recalled.values, groups=data[outer_cv_group].values
    ):
        ...
        if np.mean(data.recalled.values[test_idx]) in [0.0, 1.0]:
            auc = np.nan
            auc_null = None
            p = np.nan
            fp = None
            ...
            auc, auc_null, p = post_hoc_permutation(
                y_true=data.recalled.values[test_idx], y_score=predictions
            )
            fp, tp, _ = roc_curve(data.recalled.values[test_idx], predictions)
        # compute feature activations
        activation = compute_activation(
            data.data[train_idx], inner_model.best_estimator_.coef_.flatten()
        )
...
def load_and_fit_data_encoding(path, group):
    estimator = LogisticRegression(
        penalty="l2", class_weight="balanced", solver="lbfgs", max_iter=1000
    )
    param_grid = {"C": np.power(10.0, -np.arange(6))}
...
def bordered_imshow(X, mask, line_kwargs={"linewidth": 4, "color": "k"}, **kwargs):
    ...
    for cell in masked_cells:
        ...
        if (cell[0], cell[1] - 1) not in masked_cells:
            line = plt.Line2D(
                [
                    extent[0] + (extent[1] / shape[1]) * cell[1],
                    extent[0] + (extent[1] / shape[1]) * cell[1],
                ],
                [
                    extent[2] + (extent[3] / shape[0]) * cell[0],
                    extent[2] + (extent[1] / shape[0]) * cell[0] + 1,
                ],
                **line_kwargs,
            )
            ax.add_patch(line)
        ...
        if (cell[0], cell[1] + 1) not in masked_cells:
            line = plt.Line2D(
                [
                    extent[0] + (extent[1] / shape[1]) * cell[1] + 1,
                    extent[0] + (extent[1] / shape[1]) * cell[1] + 1,
                ],
                [
                    extent[2] + (extent[3] / shape[0]) * cell[0],
        ...
        if (cell[0] + 1, cell[1]) not in masked_cells:
            line = plt.Line2D(
                ...
                [
                    extent[2] + (extent[3] / shape[0]) * cell[0] + 1,
                    extent[2] + (extent[1] / shape[0]) * cell[0] + 1,
                ],
                **line_kwargs,
            )
        ...
        if (cell[0] - 1, cell[1]) not in masked_cells:
            line = plt.Line2D(
                ...
                [
                    extent[2] + (extent[3] / shape[0]) * cell[0],
                    extent[2] + (extent[1] / shape[0]) * cell[0],
                ],
                **line_kwargs,
            )
            ax.add_patch(line)
...
def n_list_normalization(
    ...
    for subject in subject_list:
        ...
        model = LogisticRegression(
            penalty="l2", C=best_c, class_weight="balanced", solver="liblinear"
        )
        prob = perform_normed_loso_cross_validation(
            model, events=evs, powers=ts, n_lists=n_lists
        )
...
def perform_normed_loso_cross_validation(classifier, powers, events, n_lists, **kwargs):
    ...
    for sess_idx, sess in enumerate(sessions):
        ...
        insample_pow_mat = (
            insample_pow_mat.groupby("session")
            .reduce(scp.stats.zscore, dim="event", ddof=1)
            .data
        )
        ...
        outsample_pow_mat = (
            outsample_pow_mat.groupby("session")
            .apply(norm_sess_feats, n_lists=n_lists)
            .data
        )
